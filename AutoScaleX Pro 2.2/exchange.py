...
HTTP timeout BingX: кортеж (connect, read) из config — BINGX_HTTP_CONNECT_TIMEOUT, BINGX_HTTP_READ_TIMEOUT.
...
try:
    ...
    _BINGX_HTTP_TIMEOUT = (
        float(getattr(_config, "BINGX_HTTP_CONNECT_TIMEOUT", 20.0)),
        float(getattr(_config, "BINGX_HTTP_READ_TIMEOUT", 20.0)),
    )
except ImportError:
    _CB_NOTIFY_COOLDOWN = 300
    _BINGX_HTTP_TIMEOUT = (20.0, 20.0)
...
class BingXSpot:
    ...
    def __init__(self, key, secret, telegram_notifier=None):
        ...
        self._http_timeout = _BINGX_HTTP_TIMEOUT
        ...
        def _make_request():
            ...
            for attempt in range(1, max_attempts + 1):
                try:
                    ...
                    if method_upper == "GET":
                        r = self.sess.get(url, params=params_copy, headers=headers, timeout=self._http_timeout)
                    elif method_upper == "DELETE":
                        r = self.sess.delete(url, params=params_copy, headers=headers, timeout=self._http_timeout)
                    else:
                        r = self.sess.post(url, data=params_copy, headers=headers, timeout=self._http_timeout)
                    r.raise_for_status()
                    data = r.json()
                    if data.get("code") != 0:
